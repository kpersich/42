...
int main(int argc, char **argv)
{
    ...
    if (!(file = fopen(argv[1], "r")) ||
        (fscanf(file, "%d %d %c\n", &b_width, &b_heigth, &background) != 3) ||
        (!(b_width > 0 && b_width <= 300 && b_heigth > 0 && b_heigth <= 300)) ||
        (!(paper = malloc(b_width * b_heigth))))
        return (ft_perror(ERR2));
    memset(paper, background, b_width * b_heigth);
